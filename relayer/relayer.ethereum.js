...
async function main() {
    try {
        ...
        const accounts = await web3ForPolygon.eth.getAccounts()

        console.log("Listening for Burn events on Ethereum")

        const burnEvents = await web3.eth.subscribe('logs', {
            address: contractAddressForEthereum,
        ...
        burnEvents.on('data', async (event) => {
            ...
            await polContract.methods.mint(addressPadding(event.topics[1]), BigInt(event.data))
